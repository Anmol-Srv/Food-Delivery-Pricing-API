...
const generatePricings = (organizationCount, itemCount, count) => {
  ...
  return Array.from({ length: count }, () => ({
    ...
    baseDistanceInKm: faker.number.int({ min: 1, max: 20 }),
    kmPrice: parseFloat(faker.commerce.price({min: 1, max: 5})),
    fixPrice: parseFloat(faker.commerce.price({min: 5,max: 25})),
  }));
};
